...
#
# Runs the same ltex-ls-plus in the pre-push hook as in CI. In CI the job's
# own container is ghcr.io/alrayyes/ltex-cli-plus (built once by that repo,
# not fetched fresh here), so ltex-cli-plus is already on PATH and this
# script's only job there is to skip the fetch and use it. Locally it still
# ships its own JDK and is a ~300 MB download the first time, which is why
# this caches it outside the repository; after that a run over a handful of
# documents takes about ten seconds.
set -uo pipefail
