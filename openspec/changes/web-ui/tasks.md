...
- [x] 12.1 Add a frontend build stage to `Dockerfile` ahead of the Go build stage, and verify `docker build .` produces the same distroless image shape with no Node/bun in the final layer - verified for real: `docker build .` succeeds, the running container answers `/healthz` and serves `/settings`, and the final image is ~16 MB (distroless + static Go binary, nothing Node/bun-shaped baked in). Along the way, also fixed `.goreleaser.yml`'s `before.hooks` and `.github/workflows/release.yml` (added `oven-sh/setup-bun`) so a real tagged release's binary gets a freshly built frontend too, not just `docker build .` - goreleaser had no frontend step at all before this. Also added a `web` job to `ci.yml`: no job anywhere ran `cmd/hush-hush/web`'s own `check`/`test`/`lint`/`build` before this, across all of #206/#207/#215
- [x] 12.2 Update README (requirements, `PUBLIC_URL`, how to reach the web UI, frontend toolchain) and CONTRIBUTING (frontend build/test commands), and verify a reader with no prior context can follow it to build and reach the login page - both updated; the "reach the login page" half is verified only as far as curling `/settings`/`/audit-log`/`/healthz` against the real built binary (same browser-tool gap as `#206`/`#207`), not an actual browser visit
- [ ] 12.3 Migrate one real login + secret create/edit/delete + token create/revoke + audit log filter/export round trip against a running instance end to end, and verify it succeeds - **not done**. This needs an actual browser driving a real WebAuthn ceremony, which nothing in this session's toolset can do; every other "verified end to end" note above has stopped at curl/API-level checks against the real binary, never a real login. Left unchecked rather than claimed - see the session's own final summary to whoever picks this up next
- [x] 12.4 File the `hush-hush-cli` audit-log command as an issue in that repo, referencing this change's actor/token filter shape - already existed as `hush-hush-cli#100` (filed by a peer session responding to the `#214` notification); its body had come out broken (a literal unresolved file-attachment path), so messaged that repo's own session to fix it and add `#215`'s cursor-pagination shape (`after`/`limit`, the new `id` field) rather than editing another repo directly
- [ ] 12.5 Verify every sub-issue this change touched or spawned is closed referencing what shipped, and archive this OpenSpec change - blocked on 12.3: won't close this out or archive the change while its own end-to-end validation task is still open
