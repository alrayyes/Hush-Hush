...
- [ ] 1.1 Extend the distinct-consumer store query with an optional name
...
- [ ] 1.2 Add `page`/`page_size`/`q` query parameters to
...
- [ ] 1.3 Update `api/openapi.yaml`'s `/consumers` path with the new
      parameters and response shape; verify `bun run lint:api` passes.

## 2. Consumers page

- [ ] 2.1 Add `cmd/hush-hush/web/src/routes/(app)/consumers/+page.svelte`
...
- [ ] 2.2 Selecting a consumer navigates to the secrets overview with
      `used_by` set to that consumer; verify with a component test.
- [ ] 2.3 Add a nav link to the consumers page; verify it's reachable from
      every authenticated page's nav.
- [ ] 2.4 Add an axe-core assertion (WCAG 2.1 AA) to a journey test
      covering the consumers page; verify zero violations.

## 3. Compatibility check

- [ ] 3.1 Confirm `consumer-combobox`'s no-parameters call against
...
- [ ] 4.3 Manually verified against the running binary: the consumers
