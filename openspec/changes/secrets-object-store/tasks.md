...
- [ ] 10.1 Implement inject: seal a value to one or more configured recipient keys and call the create endpoint, and verify a new object appears on the server afterward
- [ ] 10.2 Verify inject runs correctly inside a CI job using only normal environment/config, with no CI-specific code path

## 11. CLI: Get Command (alrayyes/hush-hush#35)

- [ ] 11.1 Implement get: fetch and decrypt one value to stdout per call, and verify it prints the correct plaintext for an object it holds a matching key for
- [ ] 11.2 Verify get reports a clear decryption failure, not silent incorrect output, when the CLI holds no matching private key
- [ ] 11.3 Verify get runs correctly inside a CI job using only normal environment/config, with no CI-specific code path
- [ ] 11.4 Add Pact (pact-go) contract tests for create and get - a consumer test against the CLI's HTTP client producing a local pact file, and a provider verification test confirming the server satisfies it (alrayyes/hush-hush#28)

## 12. CLI: Update Command (alrayyes/hush-hush#13)

- [ ] 12.1 Implement update: seal a new value and call the update endpoint, and verify the object's server-side value changes accordingly
- [ ] 12.2 Add Pact contract tests for update, extending the same local pact file (alrayyes/hush-hush#29)

## 13. CLI: Delete Command (alrayyes/hush-hush#14)

- [ ] 13.1 Implement delete: call the delete endpoint, and verify the object is gone from the server afterward
- [ ] 13.2 Add Pact contract tests for delete, extending the same local pact file (alrayyes/hush-hush#30)

## 14. End-to-End Validation

- [ ] 14.1 Migrate one real secret from its prior source onto this service end to end, and verify its consuming pipeline still works after the switch
- [ ] 14.2 Remove the old copy of that secret and verify nothing still depends on it

## 15. Documentation

- [ ] 15.1 Write the README (requirements, install, usage, configuration) and verify a reader with no prior context can follow it to inject/get/update/delete a secret
- [ ] 15.2 Verify the README's examples contain no environment-specific assumptions
