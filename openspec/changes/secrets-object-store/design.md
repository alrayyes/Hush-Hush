...
**Listing objects is gated by the write bearer token, unlike every other read path.**
Alternative considered: leaving it unauthenticated, consistent with get, used_by, and the audit log. Rejected - those all need an id the caller already holds, so the flat-read-path reasoning ("confidentiality rests on who holds a matching private key, not on gating metadata access") doesn't extend to them discovering ids at all. Enumerating every stored object is a capability none of the existing unauthenticated reads grant, so it's gated the same as a write instead (alrayyes/hush-hush#188).
