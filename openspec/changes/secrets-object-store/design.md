...
Alternative considered: the server decrypting on behalf of a strongly-verified ephemeral CI identity (e.g. via OIDC), for objects flagged as CI-bound. Rejected once it became clear CI can simply hold its own decrypting private key, delivered through whatever secret storage its own CI platform already provides - the same pattern as any other consumer. This avoids a second trust model, a special object "kind," and an OIDC integration that isn't needed once CI is just another key holder.
